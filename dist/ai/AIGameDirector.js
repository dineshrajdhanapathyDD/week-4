...
export class AIGameDirector {
    constructor(playerProfiler, config) {
        this.decisionHistory = [];
        this.lastAnalysisTime = 0;
        this.ANALYSIS_INTERVAL = 1000; // Analyze every 1 second
        this.MAX_DECISION_HISTORY = 100;
        // AI state tracking
     ...
     */
    analyzePlayerBehavior(gameState, playerInput) {
     ...
     */
    performBehaviorAnalysis(gameState) {
    ...
    calculateGameStats(gameState) {
        ...
        return {
            ...
            foodDistance: CollisionDetector.calculateDistance(gameState.snake[0].position, gameState.food)
        };
    }
    /**
     * Assesses overall player performance
     */
    assessPerformance(gameState, profileReport) {
     ...
     */
    makeAdaptationDecisions(gameState, analysis) {
    ...
    evaluateRecoveryMode(analysis) {
        ...
        if (shouldActivateRecovery && !this.recoveryModeActive) {
            ...
            this.logDecision('Recovery mode activated', `High risk (${analysis.risk.toFixed(2)}) or stress (${analysis.profile.stressLevel.toFixed(2)}) detected`);
        }
        else if (!shouldActivateRecovery && this.recoveryModeActive) {
            this.recoveryModeActive = false;
            this.logDecision('Recovery mode deactivated', 'Player performance stabilized');
        }
    }
    /**
     * Evaluates speed adjustment needs
     */
    evaluateSpeedAdjustment(gameState, analysis) {
        ...
        if (analysis.performance.overall > this.configuration.masteryThreshold &&
            ...
            targetSpeed = Math.min(this.configuration.maxSpeedIncrease, currentSpeed + (this.configuration.adaptationSensitivity * 0.2));
        ...
        // Decrease speed for poor performance or recovery mode
            ...
            targetSpeed = Math.max(this.configuration.minSpeedDecrease, currentSpeed - (this.configuration.adaptationSensitivity * 0.15));
        ...
        if (Math.abs(targetSpeed - currentSpeed) > 0.1) {
            ...
            this.logDecision(`Speed adjusted to ${targetSpeed.toFixed(2)}`, reasoning);
        }
    }
    /**
     * Updates overall difficulty level
     */
    updateDifficultyLevel(analysis) {
        ...
        if (Math.abs(targetDifficulty - this.currentDifficultyLevel) > 0.1) {
            ...
            this.logDecision(`Difficulty level adjusted to ${targetDifficulty.toFixed(2)}`, `Based on performance score: ${performanceScore.toFixed(2)}`);
        }
    }
    /**
     * Updates performance streak counters
     */
    updatePerformanceStreaks(analysis) {
     ...
     */
    calculateOptimalFoodPlacement(gameState) {
        try {
            // Validate game state
            ...
            this.logDecision(`Food placed at (${position.x}, ${position.y}) with ${finalDifficulty} difficulty`, reasoning);
            return position;
        }
        catch (error) {
     ...
     */
    validateFoodPosition(position, gameState) {
        return position &&
            ...
            !gameState.snake.some(segment => segment.position.x === position.x && segment.position.y === position.y);
    }
    /**
     * Provides fallback food position when AI fails
     */
    getFallbackFoodPosition(gameState) {
     ...
     */
    adjustGameSpeed(currentSpeed, playerProfile) {
        ...
        if (Math.abs(newSpeed - currentSpeed) > 0.05) {
            this.logDecision(`Speed adjusted from ${currentSpeed.toFixed(2)} to ${newSpeed.toFixed(2)}`, `Reaction time: ${avgReactionTime.toFixed(0)}ms, Stress: ${stressLevel.toFixed(2)}`);
        }
        return newSpeed;
    }
    /**
     ...
     */
    validateAndClampSpeed(speed) {
        if (!this.validateSpeedInput(speed)) {
            return 1.0; // Safe fallback
        }
        return Math.max(this.configuration.minSpeedDecrease, Math.min(this.configuration.maxSpeedIncrease, speed));
    }
    /**
     * Determines if recovery mechanic should be triggered
     */
    shouldTriggerRecoveryMechanic(playerProfile) {
        const stressLevel = playerProfile.calculateStressLevel();
        ...
        if (shouldTrigger && !this.recoveryModeActive) {
            this.logDecision('Recovery mechanic triggered', `Multiple struggle indicators: stress=${stressLevel.toFixed(2)}, errors=${errorFrequency.toFixed(2)}, poor streak=${this.consecutivePoorPerformance}`);
        }
     ...
     */
    updateDecisionContext(gameState) {
    ...
    updateConfiguration(newConfig) {
        ...
        this.logDecision('Configuration updated', `New settings: ${JSON.stringify(validatedConfig)}`);
    }
    /**
     ...
     */
    validateDeterministicCore(gameState, expectedState) {
        const differences = [];
...
//# sourceMappingURL=AIGameDirector.js.map