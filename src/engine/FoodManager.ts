...
import { Position, GameState, SnakeSegment } from '../types/GameTypes';
...
export class FoodManager {
  private readonly FOOD_SCORE_VALUE = 10;
  private readonly BONUS_SCORE_MULTIPLIER = 1.5;
  private readonly BONUS_THRESHOLD = 5; // Consecutive food without collision

  /**
   * Checks if food is consumed and handles the consumption logic
   */
  public static checkFoodConsumption(gameState: GameState): FoodConsumptionResult {
   ...
   */
  private static calculateScoreIncrease(gameState: GameState): number {
   ...
   */
  public static generateOptimalFoodPosition(
    gameState: GameState, 
    aiPreference?: 'easy' | 'medium' | 'hard'
  ): Position {
   ...
   */
  private static getAllValidPositions(gameState: GameState): Position[] {
    const { width, height } = gameState.gridSize;
   ...
   */
  public static isValidFoodPosition(position: Position, gameState: GameState): boolean {
    // Check bounds
   ...
   */
  public static getFoodPlacementStats(gameState: GameState): {
    ...
    const distances = validPositions.map(pos => 
      CollisionDetector.calculateDistance(head.position, pos)
    );
   ...
   */
  public static suggestFoodPlacement(gameState: GameState): {
    position: Position;
    reasoning: string;
    difficulty: 'easy' | 'medium' | 'hard';
  } {
   ...
   */
  public static calculateCollectionEfficiency(
    foodCollected: number, 
    totalMoves: number, 
    gameTime: number
  ): number {
    if (totalMoves === 0 || gameTime === 0) return 0;
